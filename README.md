...
To avoid wasting requests, when the app is recharged or the reload button is pressed only the main request is performed, and the specific info data for every launch is then retrived from the backup file; if a value isn't contained in the backup file a request is performed, but if two or more requests need data that were already requested (there was another launch with the same info) the data are retrived from the previous request. 
To Be Determiend (TBD) and To Be Confirmed (TBC) launches are not read by the application, only upcoming GO (Ready to Go) launches are displayed.
